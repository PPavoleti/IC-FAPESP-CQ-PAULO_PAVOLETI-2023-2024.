...
def ang(vec):
    """
    Evaluate for the rotation matrixes to be used by controlled gates.
    """
    dim=len(vec)
    n=int(np.log2(dim))
    y=zeros(dim)
...
def oseq(init,fin):
    """
    creates a sequencial array ordered
    """
    v=[init]
    n=fin-init
    for i in range(n):
...
def prep(vec):
    """
    Does the encoding to perform the algorithm
    """
...
def txun(M,b,x0,t,k):
    """
    Tao Xin algorithm (unitary case)
    """
    N=len(x0)
...
def solanogen(M,mats,coefs,b,x_0,t,k):
    """
    Tao Xin algorithm (non-unitary case)
    """
    ...
    #Criando e aplicando as VT como prepração de estados
    vecV=[0]*(2**qdits)
    for i in range(L):
